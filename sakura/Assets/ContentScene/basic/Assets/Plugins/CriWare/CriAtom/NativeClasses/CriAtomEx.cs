...
public static class CriAtomEx
{
	...
	/**
	 * <summary>[Android] (ベータ機能)音声再生遅延の計測処理を有効・無効にする</summary>
	 * <param name="enabled">有効・無効切替フラグ</param>
	 * \attention
	 * 本関数はベータ機能です。製品には使用しないでください。<br/>
	 * \par 説明:
	 * Android端末の音声遅延時間推測機能の有効化・無効化を切り替えます。<br/>
	 * 有効化すると、プラグインの内部で遅延測定処理が動くようになります。<br/>
	 * <br/>
	 * 推測機能を有効にした状態で、 ::CriAtomEx::GetEstimatedSoundLatency_ANDROID 関数を呼び出すと
	 * 実行中のAndroid端末の音声再生遅延時間の推測値が取得できます。<br/>
	 * <br/>
	 * 有効化の際は本関数を直接使わず、 ::CriWareInitializer コンポーネントの ::CriAtomConfig.androidEnableAudioLatencyCheck のプロパティから行ってください。<br/>
	 * 結果を取得した後は、本関数を使用して計測処理を無効化してください。
	 * \sa CriAtomEx::GetEstimatedSoundLatency_ANDROID
	 */
	public static void SetAudioLatencyCheckEnabled_ANDROID(bool enabled)
	{
	#if !UNITY_EDITOR && UNITY_ANDROID
		criAtom_EnableSlLatencyCheck_ANDROID(enabled);
	#endif
	}
	
	/**
	 * <summary>[Android] (ベータ機能)音声再生遅延時間の推測値の取得</summary>
	 * \attention
	 * 本関数はベータ機能です。製品には使用しないでください。<br/>
	 * \par 説明:
	 ...
	 * 本関数で得られた結果は開発時のパラメタ調整のための目安とし、実際のアプリケーション上で使用しないでください。<br>
	 * \par 注意:
	 * 本関数を実行する場合、事前に遅延計測処理を有効化しておく必要があります。
	 * \sa CriWareInitializer.atomConfig CriAtomConfig.androidEnableAudioLatencyCheck
	 */
	public static int GetEstimatedSoundLatency_ANDROID()
	{
	#if !UNITY_EDITOR && UNITY_ANDROID
		return criAtom_GetSlBufferConsumptionLatency_ANDROID();
	#else
		return -1;
	#endif
	}
